...
**`AddAccountSheet`** constructs **`AccountCreationFlow`** internally. Pass **`discovery: DiscoveryClient?`** (required for `.free`, omit for `.locked`). Convenience initializer wires **`AccountCreationWizard`**.
